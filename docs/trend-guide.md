...
1. `scripts/trend_pipeline.py` batches `sis query` over the entire corpus (using `--path`/`--glob`) to collect `findings` and `correlations` JSONL output.
2. The script appends a `daily.csv` row per finding kind + correlation pattern, including `invalid_pdf_header` when a file masquerading as a PDF is rejected.
...
- `sis` binary on the PATH (the latest build is already installed on the VM).  
  - `sis version` now prints the installed version, which the pipeline uses for the `sis_version` column.
...
- `sis query` is executed once for `findings` and once for `correlations` with `--path /var/mwb/corpus --glob "*.pdf"`, so every daily subdirectory gets covered in a single command. Non-zero exit codes are logged but the JSONL lines emitted before the failure are still ingested, which keeps a few malformed files from stopping the export.
...
> **Note:** When `sis query` encounters a PDF without a `%PDF-` header, it emits an `invalid_pdf_header` finding rather than exiting. The trend pipeline now captures that finding so every malformed file still contributes to the CSV instead of halting the run.
