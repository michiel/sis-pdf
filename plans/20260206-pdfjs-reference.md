...
- Harden the hinting execution guard so it aborts before the stack overflow occurs (e.g., cap recursion/loop depth or return early when state resets) and re-run the pdf.js corpus to ensure the scan completes.  
- Compare the corpus output to the new structural/stream findings and document the expected noise so future runs only flag meaningful regressions.
...
- **High ObjStm count pattern**: the debug corpus keeps logging `high_objstm_count` warnings. Consider adding or extending ObjStm findings to aggregate counts (e.g., `objstm.count_anomaly` or `objstm.torture`) so ObjStm-rich files are clearly marked when they also show mismatched headers or suspicious filters.
