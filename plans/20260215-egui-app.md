...
1. **Triage-first** — default view answers "is this malicious?" in under 3 seconds
2. **Everything is linked** — findings → objects → graph nodes → evidence are all cross-referenced and clickable
3. **Progressive disclosure** — summary → table → detail → raw bytes
4. **Keyboard-friendly** — power users never need the mouse for common workflows
5. **Export-ready** — any data view can be copied or exported
6. **CLI-faithful** — GUI query semantics match CLI exactly; no silent divergence
...
```
Drop PDF → Triage (is it bad?) → Investigate (how is it bad?) → Extract (get IOCs/payloads) → Report (share findings)
```

## Milestone Cuts

### Milestone 1 — MVP (single PR target)
...
```
┌─────────────────────────────────────────────────────────┐
│ File▾  [sample1.pdf] [sample2.pdf]           ⚙  🌙/☀  │  ← top-bar
├─────────────────────────────────────────────────────────┤
│ report.pdf │ 482KB │ v1.7 │ 127 objs │ 2C 5H 3M 1L 12I │ 4⛓ │  ← workspace-top-bar
├────┬────────────────────────────────────────────────────┤
│    │                                                    │
│ 📊 │  ┌─ Findings ──────────────────┐ ┌─ Detail ─────┐ │
│ 🔍 │  │ [C▾][H▾][M▾][L▾][I▾] 🔎   │ │ JS_OBFUSC... │ │
│ 📋 │  │ Sev Conf Kind    Surface   │ │              │ │
│ 🔗 │  │ ██C Cert js_obf  JavaScript│ │ Description  │ │
│ 📦 │  │ ██H Strg launch  Actions   │ │ Evidence     │ │
│ 🌐 │  │ ██M Prob xfa_sc  Forms     │ │ Objects [→]  │ │
│ ⚡ │  │                            │ │ Reader Impact│ │
│    │  └─────────────────────────────┘ └──────────────┘ │
│    │                                                    │
├────┴────────────────────────────────────────────────────┤
│ > query: findings --where "severity == 'Critical'"      │  ← command bar (M2)
└─────────────────────────────────────────────────────────┘
```
...
- Center: PDF tab labels — each open PDF adds a navigation item (max per milestone)
...
| Confidence | yes      | Certain → Heuristic                                |
...
- Click row → opens Detail panel for that finding
- Double-click row → opens Detail panel AND highlights related objects in graph (if open, M3)
- Right-click → context menu: "Show in graph" (M3), "Copy as JSON", "Filter to this kind"
- Multi-select (Shift+click) → bulk operations (export selected)
...
- **Evidence:** Each `EvidenceSpan` shown with:
  ...
  - "View in hex viewer" link → opens Evidence panel at that offset (M2)
- **Related Objects:** List of object IDs, each clickable → opens Object Inspector
- **Chain Membership:** If this finding is part of a chain, show the chain with this finding highlighted
- **Reader Impacts:** Table of Reader → Version → Impact Level → Notes
- **CVE References:** Linked CVE IDs (clickable → open in browser on native, show details on WASM)
...
- **Query source:** in-memory sort; chain→finding lookup via `chain.findings` IDs matched against `Report.findings`
...
**List view (M1)** — table of chains sorted by score:

| Score | Trigger    | Action     | Payload    | Findings | Path                              |
|-------|------------|------------|------------|----------|-----------------------------------|
| 0.95  | OpenAction | JavaScript | heap_spray | 3        | Catalog→OpenAction→JS→Stream      |

**Flow view (M2)** — visual representation of selected chain as a horizontal flow diagram:

```
[Trigger: OpenAction] ──→ [Action: JavaScript] ──→ [Payload: heap_spray]
     obj 1 0                    obj 8 0                  obj 12 0
```

Each node is clickable → opens Object Inspector.

**Interactions:**

- Select chain → highlights its nodes in the ORG graph (if open, M3)
- Click finding ID in chain → jumps to that finding in the Findings table
- "Explain chain" button → shows `reasons` in a tooltip/panel
...
- Stream content (if present):
  ...
  - Raw view (M2)
  - Filter chain info (e.g., FlateDecode → ASCIIHexDecode)
  - Length, entropy
...
- Click reference → navigates to that object (M2 adds back/forward navigation stack)
- Click finding → opens Detail panel
- "Show in graph" → highlights in ORG graph (M3)
- "Extract stream" → downloads decoded stream content (WASM: browser download, native: file save dialog) (M2)
- "Copy as JSON" → copies object dict to clipboard
...
- **Empty state:** show fields as "N/A" or "not present" — metadata is always partially available
- **Error state:** N/A
...
- **Temporal Signals:** If available — creation/modification patterns, timezone analysis
- **Supply Chain Indicators:** Tool chain detection from metadata

**Interactions:**

- Click deviation → shows finding if one exists
...
- Click node → select, show info tooltip
- Double-click node → open in Object Inspector
...
- "Show only chain path" — filter to nodes/edges in a selected chain
- Depth slider — show only N hops from selected node
...
- Search for object ID → center and highlight
...
- Click "view evidence" on a finding → opens hex viewer at that offset
- Click "view raw" on an object stream → shows decoded stream bytes
- From command bar: `stream 8 0 --raw` output → hex view
...
- `object <num> [gen]` (aliases: `obj`, `o`) — inspect object
- `stream <num> [gen] [--raw|--hexdump|--decode]` — view stream
- `ref <num> [gen]` (alias: `references`) — find references
...
- Shell piping (`|`) — no shell access in browser
- File redirection (`>`) — replaced by browser download
- `--extract-to` — replaced by browser download

**GUI-specific commands (not in CLI):**

- `goto <num> [gen]` → open Object Inspector for that object
- `graph focus <num> [gen]` → center graph on object (M3)
- `highlight chain:<id>` → highlight chain in graph (M3)
- `export report` → trigger report download
...
- `selected_finding: Option<usize>` — index into filtered findings
- `selected_object: Option<(u32, u16)>` — obj num + gen
- `selected_chain: Option<String>` — chain ID
- `graph_focus: Option<(u32, u16)>` — object to center in graph
...
Lightweight, opt-in instrumentation for tuning UI performance and understanding analyst behavior. No external reporting — metrics are logged to console (native) or available via a debug panel.

**Performance metrics:**

- Frame time (p50, p95, p99) per panel — detect jank
...
1. Drop PDF
2. See summary bar → "2 Critical, 5 High" + ML verdict badge
3. Click Critical badge → findings filtered to Critical
4. Scan titles → "JavaScript heap spray detected", "OpenAction automatic execution"
5. Decision: escalate / quarantine / safe

### Workflow 2: Exploit Chain Investigation (2-5 min)

1. Drop PDF → see "4 chains" in summary bar
2. Open Chains panel → sort by score
3. Select highest-scoring chain → see flow: Trigger(OpenAction) → Action(JavaScript) → Payload(heap_spray)
4. Click payload node → Object Inspector shows decoded JavaScript
5. Open ORG graph → chain path highlighted (M3)
6. Trace through graph to understand object relationships
7. Click evidence on JS finding → hex viewer shows obfuscated code (M2)

### Workflow 3: IOC Extraction (1-3 min)

1. After investigation, extract indicators
2. Command bar: `urls` → see all URLs in document (M2)
3. Command bar: `javascript` → see all JavaScript, click "Extract" to download
4. Command bar: `embedded` → list embedded files with types and hashes
5. Select findings → "Copy as JSON" → paste into SIEM/ticketing system
6. Command bar: `export report` → download full JSON report

### Workflow 4: PDF Structure Analysis (forensic deep-dive)

1. Open Metadata panel → check Creator/Producer for known exploit kit signatures
2. Check revision timeline → see if incremental updates added suspicious objects
3. Open Object Inspector → browse objects by type
4. Filter to Action objects → check for OpenAction, AA entries
5. Follow references through object graph
6. Open ORG graph → filter to suspicious edges only (M3)
7. Command bar: `canonical-diff` → compare revisions (M2)
8. Command bar: `cycles` → check for reference cycles (common evasion) (M2)
...
| Chains flow         | Flow diagram renders trigger→action→payload with clickable nodes; nodes open Object Inspector |
...
- **Hex viewer:** Lazy loading — only render visible byte range; 1 MB page size
