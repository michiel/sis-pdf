...
- [ ] Replace CLI unwraps that touch real user inputs or output formatting:
  - `src/main.rs` (REPL helpers, batch report handling)
  - `src/commands/query.rs` (SWF metadata version, JSON output parsing, tests that currently `unwrap()` but could use `expect` with context)
  - `src/commands/query/readable.rs` (table builders that unwrap array metadata)
