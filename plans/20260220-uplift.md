...
Compatibility rules:
- Add `chain_schema_version: u32` to chain-bearing serialised outputs (initial value: `2`).
...
- Add schema compatibility tests in `crates/sis-pdf-core/tests/`:
  - serialise current chain model and assert `chain_schema_version == 2`,
  - deserialise previous-schema fixture and assert additive-field defaults.
...
- stable context keys when present: `chain.stage`, `action_type`, `action_target`, `edge.reason`
...
11. Schema compatibility tests assert versioning and additive-field defaults for chain JSON output.
12. Feature performance budgets are measured and meet defined gates.
