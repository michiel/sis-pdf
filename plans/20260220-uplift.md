...
Completed after initial four batches:

- V9 chain narrative composition is now populated in synthesis (`chain.narrative`) and covered by deterministic tests in `chain_render` and `chain_grouping`.
- Performance budget tests added:
  - `graph_layout_staged_dag_budget`
  - `critical_path_budget`
  - `taint_overlay_mapping_budget`
  - `diff_large_input_budget`
- `sis diff` fingerprinting now normalises `action_target` by stripping query strings and fragments before matching.
- Fixture-family drift guards added to corpus-captured regressions:
  - complex/distributed/fragmented chain (`modern-openaction-staged-38851573.pdf`)
  - revision-shadow (`modern-renderer-revision-8d42d425.pdf`)
  - multi-reader divergence (`modern-openaction-staged-38851573.pdf`, `modern-gated-supplychain-9ff24c46.pdf`)
- Schema compatibility gate expanded with a committed v1 report fixture (`tests/fixtures/report_v1_chain_schema.json`) and additive-field default assertions (`tests/report_schema_compat.rs`).
- CI quality gate workflow added at `.github/workflows/quality-gates.yml` for schema/fixture/budget/runtime-profile checks.
- Added dedicated synthetic uplift fixtures and deterministic regression coverage:
  - `tests/fixtures/uplift/complex_distributed_fragmented_chain.json`
  - `tests/fixtures/uplift/revision_shadow_chain.json`
  - `tests/fixtures/uplift/multi_reader_divergence_chain.json`
  - test module: `tests/uplift_synthetic_fixtures.rs`
- Added RSS budget enforcement to `diff_large_input_budget` (Linux `/proc/self/status` `VmRSS`), with gate `<= 300 MB`.
- Updated user-facing documentation for:
  - `sis diff` matching semantics and exit-code behaviour (`docs/forensic-workflows.md`)
  - scan JSON schema-version expectations (`docs/forensic-workflows.md`, `docs/index.md`)
- Added query payload stability gate:
  - `format_json_and_jsonl_keep_stable_top_level_keys`

Recorded baseline notes:

- `cargo test -p sis-pdf diff_large_input_budget -- --nocapture` currently passes in ~1.49s for 100k findings per side.
- Runtime profile command remains:
...
- Latest measured output for the runtime profile fixture:
  - `parse`: `0 ms`
  - `detection`: `1 ms`
  - `total_duration_ms`: `6 ms`
- Latest full sweep (next-steps implementation):
  - `cargo test -p sis-pdf-core --test uplift_synthetic_fixtures`: `3 passed`
  - `cargo test -p sis-pdf diff_large_input_budget -- --nocapture`: `passed` (time + RSS gate)
  - `cargo test -p sis-pdf-core --test corpus_captured_regressions`: `20 passed`
  - `cargo test -p sis-pdf-core --test chain_grouping`: `9 passed`
  - runtime profile rerun: `parse=0 ms`, `detection=1 ms`, `total_duration_ms=10 ms`

Baseline delta note:

- `corpus_captured_noisy_likely_noise_bucket_stays_stable` now supports both historical and current behaviour:
  - historical: `content_stream_anomaly` with `likely_noise` metadata
  - current: `missing_pdf_header` low/probable fallback without noisy-bucket metadata

## Implementation readiness review (2026-02-20)

Readiness outcome: implementable with current workspace structure.

Validated target modules:
- Core chain and graph synthesis modules exist and are separable for phased delivery:
  - `crates/sis-pdf-core/src/chain.rs`
  - `crates/sis-pdf-core/src/chain_synth.rs`
  - `crates/sis-pdf-core/src/chain_render.rs`
  - `crates/sis-pdf-core/src/event_graph.rs`
  - `crates/sis-pdf-core/src/taint.rs`
- GUI integration points exist for planned uplift:
  - `crates/sis-pdf-gui/src/app.rs`
  - `crates/sis-pdf-gui/src/analysis.rs`
  - `crates/sis-pdf-gui/src/graph_data.rs`
  - `crates/sis-pdf-gui/src/graph_layout.rs`
  - `crates/sis-pdf-gui/src/panels/chains.rs`
  - `crates/sis-pdf-gui/src/panels/graph.rs`
  - `crates/sis-pdf-gui/src/panels/detail.rs`
  - `crates/sis-pdf-gui/src/panels/hex_viewer.rs`
  - `crates/sis-pdf-gui/src/panels/objects.rs`
- CLI command surface is ready for `sis diff` integration:
  - `crates/sis-pdf/src/commands/mod.rs`
...
2. Batch 1 (core schema + chain model):
- Implement schema/version field on report envelope.
- Land V1 and V2 data-model changes with deterministic tests.
- Keep GUI untouched in this batch.

3. Batch 2 (taint + evidence navigation + object heatmap):
- Implement V3 taint model and provenance mapping.
- Implement V7 and V8 GUI workflow improvements.
- Add non-render logic tests and manual verification notes.

4. Batch 3 (chain panel + staged graph + critical path + MITRE + taint overlay):
- Implement V6, V4, V5, V10, V12 with performance budget tests.
- Add staged layout and critical-path deterministic unit coverage.

5. Batch 4 (revision panel + annotations + CLI diff):
- Implement V11, V13, V14.
- Add diff fingerprint collision tests and large-input budget checks.

6. Fixtures and quality gates:
- Add fixture families defined in this plan before closing Batch 3.
- Enforce budget regression gate and schema compatibility gate before final merge.
