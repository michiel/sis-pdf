...
- Step 1 (canonical metadata) is complete: detectors all share the canonical view, `sis query canonical-diff` reports the sampled removals/renames, and the canonical counts feed into the structural summary.
- Step 2 (telemetry schema) is complete: findings now emit `impact`, `reader_impacts`, `action_*` and the docs/plan mention the new fields while `docs/threat-intel-tracker.md` records fixtures, owners, and CVEs.
- Step 3 (reader-context scoring) is complete: reader-profile tests cover cap behaviour, the SLO gate reads `docs/performance-data/profile-launch-cve.json`, and the canonical diff CLI/docs make the normalized view discoverable.
- Step 4 (emerging primitives, fixtures, cadence) remains in progress—the heuristics, tracker automation, and weekly cadence updates described above still need follow-up before we can claim full coverage.

## Remaining items

1. **Emerging primitives & fixtures** – finish the JBIG2, gvar, and zero-click heuristics referenced in `plans/20260201-next-steps.md`, link each CVE/finding to a fixture (`crates/sis-pdf-core/tests/fixtures` and `crates/font-analysis/tests/fixtures/cve`), and keep `docs/threat-intel-tracker.md` in sync with owner/severity/impact/confidence metadata.
2. **CVE automation refresh** – refresh `tools/cve-update`/`.github/workflows/cve-update.yml` so the job fetches only the last seven days, handles retries/idempotency, broadens its keyword coverage beyond fonts, surfaces 404/429 errors, and seeds the tracker with ready-to-triage rows.
3. **Cadence & docs** – continue populating `plans/20260201-threat-intel-cadence.md` with the automation status, ensure release notes cite the new heuristics, and keep documenting predicate queries (e.g., `meta.cve`, `meta.attack_surface`) that analysts rely on.
4. **Feature-extraction stability** – revisit `crates/sis-pdf-core/tests/feature_extraction.rs` once canonical metadata settles to restore the `swf_actionscript_count`, `max_chain_length`, and `high_entropy_stream_count` expectations that previously failed.
