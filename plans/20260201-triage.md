...
- Step 4 (emerging primitives, fixtures, cadence) is complete—the new JBIG2/zero-click/gvar heuristics emit CVE metadata, the tracker rows call out fixtures and owners, and the automation now feeds `docs/threat-intel-tracker.md` with placeholder CVE rows that analysts can triage.

## Next frontiers

1. **Weekly tracker and release hygiene** – keep the automation-sourced rows from `docs/threat-intel-tracker.md` assigned to owners, update `docs/findings.md` as heuristics land, and drop the `scripts/threat_intel_summary.py` output into release notes so analysts can quickly see what the automation added.
2. **Operational SLO monitoring** – rerun `sis scan --runtime-profile` against the canonical CVE fixtures whenever filter/CDR heuristics change so `docs/performance.md` and `docs/performance-data/profile-launch-cve.json` stay in sync with the SLO gate in `crates/sis-pdf-core/tests/runtime_profile.rs`.
3. **Heuristic expansion backlog** – let the tracker rows seeded by `cve-update` guide which attack surfaces still need fixtures and detectors (zero-click JBIG2 clones, new font variations, action chaining updates) and keep the metadata contract in `docs/query-predicates.md` current for analysts.
