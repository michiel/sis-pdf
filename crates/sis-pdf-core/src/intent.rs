...
pub fn apply_intent(findings: &mut [Finding]) -> IntentSummary {
    ...
    for sig in signals {
        let entry = buckets
            .entry(sig.bucket)
            .or_insert_with(|| IntentBucketSummary {
                bucket: sig.bucket,
                score: 0,
    ...
    for f in findings.iter_mut() {
        ...
        if let Some((bucket, score)) = best_bucket {
            ...
            f.meta.insert(
                "intent.confidence".into(),
                format!("{:?}", score_confidence(score)),
            );
        }
    }
...
fn signals_from_finding(f: &Finding) -> Vec<IntentSignal> {
    let mut out = Vec::new();
    let fid = if f.id.is_empty() {
        None
    } else {
        Some(f.id.clone())
    };
    match f.kind.as_str() {
        "submitform_present" => out.push(signal(
            IntentBucket::DataExfiltration,
            3,
            "SubmitForm action",
            fid.clone(),
        )),
        "uri_present" => out.push(signal(
            IntentBucket::DataExfiltration,
            2,
            "URI action",
            fid.clone(),
        )),
        "gotor_present" => out.push(signal(
            IntentBucket::DataExfiltration,
            2,
            "GoToR action",
            fid.clone(),
        )),
        "launch_action_present" => out.push(signal(
            IntentBucket::SandboxEscape,
            3,
            "Launch action",
            fid.clone(),
        )),
        "open_action_present" => out.push(signal(
            IntentBucket::SandboxEscape,
            1,
            "OpenAction trigger",
            fid.clone(),
