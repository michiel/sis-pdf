...
fn reader_context_caps_severity_for_javascript() {
    ...
    assert!(finding.reader_impacts[1]
        .note
        .as_deref()
        .unwrap_or("")
        .contains("pdfium"));
    assert!(finding.reader_impacts[2]
        .note
        .as_deref()
        .unwrap_or("")
        .contains("preview"));
    ...
    assert_eq!(
        finding.meta.get("reader.impact.pdfium"),
...
fn reader_context_notes_only_when_severity_changes() {
    ...
    assert!(finding
        .reader_impacts
        .iter()
        .all(|impact| impact.severity == Severity::Medium));
    assert!(finding
        .reader_impacts
        .iter()
        .all(|impact| impact.note.is_none()));
    assert_eq!(
        finding.meta.get("reader.impact.summary"),
